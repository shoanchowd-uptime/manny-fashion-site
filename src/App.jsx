...
/** Two pages, no router. "#/about" is the About page, anything else is home.
 *  In-page anchors like "#catalog" still work because they aren't "#/about". */
function useRoute() {
  const read = () => (window.location.hash === "#/about" ? "about" : "home");
...
function Eyebrow({ children, centered = true }) {
  return (
    <div style={{ display: "flex", justifyContent: centered ? "center" : "flex-start", alignItems: "center", gap: 16, marginBottom: 20 }}>
      <div style={{ width: 40, height: 1, background: C.accent }} />
      <p style={{ fontFamily: F.body, fontSize: 11, fontWeight: 400, color: C.accent, letterSpacing: "0.2em", textTransform: "uppercase" }}>{children}</p>
      {centered && <div style={{ width: 40, height: 1, background: C.accent }} />}
    </div>
  );
}
...
function Nav({ route }) {
  ...
  const links = route === "about"
    ? [{ label: "Catalog", href: "#catalog" }, { label: "Contact", href: "#connect" }]
    : [{ label: "Catalog", href: "#catalog" }, { label: "Contact", href: "#connect" }, { label: "About", href: "#/about" }];
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: s ? "rgba(245,240,235,0.92)" : "transparent", backdropFilter: s ? "blur(16px)" : "none", borderBottom: s ? `1px solid ${C.border}` : "none", transition: "all 0.5s ease", padding: "0 clamp(18px,6vw,80px)" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: s ? 56 : 72, transition: "height 0.5s ease" }}>
        <a href="#/" style={{ fontFamily: F.display, fontSize: isMobile ? 18 : 24, fontWeight: 500, color: C.dark, textDecoration: "none", letterSpacing: "0.08em", textTransform: "uppercase" }}>Manny Fashion</a>
        <div style={{ display: "flex", gap: isMobile ? 12 : 30, alignItems: "center" }}>
          {!isMobile && links.map(l => (
            <a key={l.label} href={l.href} style={{ fontFamily: F.body, fontSize: 11, fontWeight: 400, color: C.muted, textDecoration: "none", letterSpacing: "0.14em", textTransform: "uppercase", transition: "color 0.3s" }}
...
function Hero() {
  ...
  return (
    <section id="top" style={{ padding: isMobile ? "96px 20px 44px" : "140px clamp(24px,6vw,80px) 90px", background: C.bg, position: "relative" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", width: "100%", display: "grid", gridTemplateColumns: isTablet ? "1fr" : "1.05fr 1fr", gap: isMobile ? 36 : 72, alignItems: "center" }}>
        <div style={{ opacity: 0, animation: "fadeIn 1.2s ease 0.2s forwards" }}>
          <Eyebrow centered={false}>New York — Est. 1989</Eyebrow>
          <h1 style={{ fontFamily: F.display, fontSize: isMobile ? "38px" : "clamp(40px,5vw,66px)", fontWeight: 400, color: C.dark, lineHeight: 1.08, letterSpacing: "-0.01em", margin: "0 0 20px" }}>
            Church suits, hats, and<br /><em style={{ fontStyle: "italic", color: C.accentDeep }}>occasion wear</em><br />for every woman.
          </h1>
          <p style={{ fontFamily: F.body, fontSize: isMobile ? 15 : 16, fontWeight: 300, color: C.muted, lineHeight: 1.8, maxWidth: 460, marginBottom: 30 }}>
            Wholesale for boutique owners, and direct to you. Tally Taylor, Liorah Knit, Ben Marc, Donna Vinci and more — misses through plus sizes.
          </p>
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <a href={PHONE_TEL} onClick={() => track("Contact", { method: "phone", placement: "hero" })}
              style={{ fontFamily: F.body, fontSize: 12, fontWeight: 400, color: C.white, background: C.dark, padding: isMobile ? "16px 26px" : "16px 34px", textDecoration: "none", letterSpacing: "0.14em", textTransform: "uppercase", transition: "background 0.3s" }}
            ...
            <a href={WA} target="_blank" rel="noopener" onClick={() => track("Contact", { method: "whatsapp", placement: "hero" })}
              style={{ fontFamily: F.body, fontSize: 12, fontWeight: 400, color: C.accent, padding: "16px 26px", textDecoration: "none", letterSpacing: "0.14em", textTransform: "uppercase", border: `1px solid ${C.accent}`, transition: "all 0.3s" }}
          ...
          <a href="#catalog" style={{ display: "inline-block", marginTop: 26, fontFamily: F.body, fontSize: 11, fontWeight: 400, color: C.muted, textDecoration: "none", letterSpacing: "0.14em", textTransform: "uppercase", borderBottom: `1px solid ${C.border}`, paddingBottom: 3 }}
          ...
          >Browse the collection below</a>
        </div>
        <div style={{ opacity: 0, animation: "fadeIn 1.2s ease 0.5s forwards", position: "relative", width: "100%", maxWidth: isTablet ? 320 : 440, margin: isTablet ? "0 auto" : 0, overflow: "hidden" }}>
...
function TrustedBy() {
  const brands = ["Tally Taylor", "Liorah Knit", "Ben Marc", "Donna Vinci", "Lisa Rene", "Nubiano", "Lily & Taylor", "Dorinda Clark Cole", "Elite Champagne", "GMI", "Terramina", "Chancele"];
  const doubled = [...brands, ...brands];
  return (
    <section style={{ padding: "28px 0", background: C.deep, overflow: "hidden" }}>
      <div style={{ textAlign: "center", marginBottom: 14, padding: "0 20px" }}>
...
function Catalog() {
  ...
  const RetailCard = ({ p }) => (
    <div style={{ background: C.white, border: `1px solid ${C.border}`, transition: "all 0.4s" }}
      ...
      <div style={{ position: "relative", aspectRatio: "3/4", background: C.bgAlt, overflow: "hidden" }}>
        ...
        {p.tag && (
          <span style={{ position: "absolute", top: 12, left: 12, background: C.dark, color: C.white, fontFamily: F.body, fontSize: 9, fontWeight: 400, letterSpacing: "0.16em", textTransform: "uppercase", padding: "6px 11px" }}>{p.tag}</span>
        )}
      </div>
      <div style={{ padding: isMobile ? "16px 14px 20px" : "20px 22px 24px" }}>
        <h3 style={{ fontFamily: F.display, fontSize: isMobile ? 16 : 19, fontWeight: 400, color: C.dark, marginBottom: 6, lineHeight: 1.3 }}>{p.name}{p.style ? ` — ${p.style}` : ""}</h3>
        <p style={{ fontFamily: F.body, fontSize: 15, fontWeight: 400, color: C.accent, marginBottom: 8 }}>{typeof p.price === "number" ? `$${p.price.toFixed(2)}` : p.price}</p>
        {p.color && <p style={{ fontFamily: F.body, fontSize: 10, color: C.mutedLight, letterSpacing: "0.1em", textTransform: "uppercase", lineHeight: 1.6 }}>{p.color}</p>}
        <a href={WA} target="_blank" rel="noopener" onClick={() => track("Lead", { placement: "product", product: p.name })}
          style={{ display: "block", textAlign: "center", marginTop: 14, fontFamily: F.body, fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: C.accent, textDecoration: "none", border: `1px solid ${C.border}`, padding: "10px 0" }}>Check availability</a>
      </div>
    </div>
  );
  ...
  return (
    <section id="catalog" style={{ padding: isMobile ? "56px 20px 72px" : "84px clamp(24px,6vw,80px) 110px", background: C.white }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <FadeIn>
          <Eyebrow>In stock now</Eyebrow>
          <h2 style={{ fontFamily: F.display, fontSize: isMobile ? 32 : "clamp(32px,4.5vw,50px)", fontWeight: 400, color: C.dark, textAlign: "center", marginBottom: 14, lineHeight: 1.15 }}>Browse the <em style={{ color: C.accentDeep }}>collection</em></h2>
          <p style={{ fontFamily: F.body, fontSize: 15, fontWeight: 300, color: C.muted, lineHeight: 1.8, maxWidth: 520, margin: "0 auto 36px", textAlign: "center" }}>Tap any piece to check sizes and colors on WhatsApp. Wholesale buyers, ask for the line sheet.</p>
        </FadeIn>

        <FadeIn delay={0.1}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: isMobile ? 28 : 44, borderBottom: `1px solid ${C.border}` }}>
            {["retail", "wholesale"].map(m => (
              <button key={m} onClick={() => { setMode(m); setShown(12); }} style={{
            ...
            <div style={{ textAlign: "center", padding: isMobile ? "48px 24px" : "72px 24px", background: C.bg, border: `1px solid ${C.border}` }}>
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                <a href={WA} target="_blank" rel="noopener" onClick={() => track("Lead", { placement: "catalog_empty", mode })}
                  style={{ fontFamily: F.body, fontSize: 11, fontWeight: 400, color: C.white, background: C.accent, padding: "14px 32px", textDecoration: "none", letterSpacing: "0.14em", textTransform: "uppercase" }}>Message on WhatsApp</a>
                <a href={PHONE_TEL} onClick={() => track("Contact", { method: "phone", placement: "catalog_empty" })}
                  style={{ fontFamily: F.body, fontSize: 11, fontWeight: 400, color: C.dark, border: `1px solid ${C.dark}`, padding: "14px 32px", textDecoration: "none", letterSpacing: "0.14em", textTransform: "uppercase" }}>Call instead</a>
              </div>
            </div>
          </FadeIn>
        ) : (
          <>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(auto-fill, minmax(230px, 1fr))", gap: isMobile ? 12 : 22 }}>
              {visible.map((p, i) => (
                <FadeIn key={p.id || i} delay={Math.min(i, 8) * 0.05}>
            ...
            {shown < items.length && (
              <div style={{ textAlign: "center", marginTop: 40 }}>
                <button onClick={() => setShown(s => s + 12)}
                  style={{ fontFamily: F.body, fontSize: 11, fontWeight: 400, color: C.dark, background: "transparent", border: `1px solid ${C.dark}`, padding: "15px 40px", cursor: "pointer", letterSpacing: "0.14em", textTransform: "uppercase" }}>
...
function Connect() {
  ...
  return (
    <section id="connect" style={{ padding: isMobile ? "72px 20px" : "110px clamp(24px,6vw,80px)", background: C.bg }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: isTablet ? "1fr" : "1fr 1fr", gap: isMobile ? 48 : 100, alignItems: "start" }}>
          <div>
            <FadeIn>
              <Eyebrow centered={false}>Connect</Eyebrow>
              <h2 style={{ fontFamily: F.display, fontSize: isMobile ? 32 : "clamp(32px,4.5vw,50px)", fontWeight: 400, color: C.dark, lineHeight: 1.12, marginBottom: 20 }}>Every message<br />answered <em style={{ color: C.accentDeep }}>personally</em></h2>
...
function AboutPage() {
  ...
  return (
    <>
      <section style={{ padding: isMobile ? "104px 20px 64px" : "150px clamp(24px,6vw,80px) 100px", background: C.white }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <FadeIn>
            <Eyebrow>The Journey</Eyebrow>
            <h1 style={{ fontFamily: F.display, fontSize: isMobile ? 34 : "clamp(34px,4.6vw,54px)", fontWeight: 400, color: C.dark, lineHeight: 1.12, textAlign: "center", marginBottom: 16 }}>From Comilla to <em style={{ color: C.accentDeep }}>New York</em></h1>
            <p style={{ fontFamily: F.body, fontSize: 15, fontWeight: 300, color: C.muted, lineHeight: 1.85, maxWidth: 560, margin: isMobile ? "0 auto 44px" : "0 auto 68px", textAlign: "center" }}>A three-decade journey through New York's fashion industry, built one relationship at a time.</p>
          </FadeIn>
          ...
          <FadeIn delay={0.3}>
            <div style={{ textAlign: "center", maxWidth: 640, margin: isMobile ? "48px auto 0" : "80px auto 0" }}>
              ...
              <p style={{ fontFamily: F.body, fontSize: 12, color: C.accent, letterSpacing: "0.14em", textTransform: "uppercase" }}>Manny Chowdhury</p>
            </div>
          </FadeIn>
        </div>
      </section>

      <section style={{ padding: isMobile ? "64px 20px" : "100px clamp(24px,6vw,80px)", background: C.bg }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <FadeIn>
            <Eyebrow>What we carry</Eyebrow>
            <h2 style={{ fontFamily: F.display, fontSize: isMobile ? 30 : "clamp(30px,4.2vw,46px)", fontWeight: 400, color: C.dark, textAlign: "center", marginBottom: isMobile ? 36 : 56, lineHeight: 1.15 }}>Curated for <em style={{ color: C.accentDeep }}>every</em> occasion</h2>
          </FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : isTablet ? "repeat(2,1fr)" : "repeat(3,1fr)", gap: 20 }}>
            {cards.map((c, i) => (
              <FadeIn key={i} delay={i * 0.12}>
                <div style={{ background: C.white, padding: isMobile ? "32px 24px" : "44px 32px", height: "100%", border: `1px solid ${C.border}` }}>
          ...
          <FadeIn delay={0.3}>
            <div style={{ textAlign: "center", marginTop: 48 }}>
              <a href="#catalog" style={{ fontFamily: F.body, fontSize: 12, fontWeight: 400, color: C.white, background: C.dark, padding: "16px 40px", textDecoration: "none", letterSpacing: "0.14em", textTransform: "uppercase", display: "inline-block" }}>See what's in stock</a>
            </div>
          </FadeIn>
        </div>
      </section>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 32, marginBottom: 36 }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 18, marginTop: 16 }}>
              <a href="#catalog" style={{ fontFamily: F.body, fontSize: 11, color: "rgba(255,255,255,0.55)", textDecoration: "none", letterSpacing: "0.1em", textTransform: "uppercase" }}>Catalog</a>
              <a href="#/about" style={{ fontFamily: F.body, fontSize: 11, color: "rgba(255,255,255,0.55)", textDecoration: "none", letterSpacing: "0.1em", textTransform: "uppercase" }}>About</a>
              <a href="#connect" style={{ fontFamily: F.body, fontSize: 11, color: "rgba(255,255,255,0.55)", textDecoration: "none", letterSpacing: "0.1em", textTransform: "uppercase" }}>Contact</a>
            </div>
          </div>
